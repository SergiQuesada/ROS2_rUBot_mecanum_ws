...
class RobotSelfControl(Node):
    ...
    def laser_callback(self, scan):
        ...
        if not custom_range:
            return
        # choose the closest reading explicitly by distance
        closest_distance, angle_closest_distance = min(custom_range, key=lambda x: x[0])
        ...
        # Holonomic reaction: strafe away from obstacle
        lateral_speed = self._rotationSpeed * self._speedFactor  # lateral speed when strafing
        forward_speed = self._forwardSpeed * self._speedFactor

        def lateral_direction_for_angle(angle_deg):
            # angle < 0 -> obstacle on right -> move left (positive y)
            # angle > 0 -> obstacle on left  -> move right (negative y)
            return 1.0 if angle_deg < 0 else -1.0

        # React to obstacle
        if closest_distance < self._distanceLimit:
            if zone == "FRONT":
                # Back up and strafe away from the exact obstacle side
                self._msg.linear.x = -forward_speed
                self._msg.linear.y = lateral_speed * lateral_direction_for_angle(angle_closest_distance)
                self._msg.angular.z = 0.0
            elif zone == "LEFT":
                # Obstacle on left -> strafe right
                self._msg.linear.x = 0.0
                self._msg.linear.y = -lateral_speed
                self._msg.angular.z = 0.0
            elif zone == "RIGHT":
                ...
                # Obstacle behind: move forward and slightly strafe away
                self._msg.linear.x = forward_speed
                self._msg.linear.y = lateral_speed * lateral_direction_for_angle(angle_closest_distance)
                self._msg.angular.z = 0.0
            else:
            ...
            # No close obstacle: proceed forward straight
            self._msg.linear.x = forward_speed
            self._msg.linear.y = 0.0
            self._msg.angular.z = 0.0
